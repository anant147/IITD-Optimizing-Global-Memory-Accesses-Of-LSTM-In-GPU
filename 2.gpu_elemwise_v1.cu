                                           ...
                                           int rindex, int numHiddenUnits)
{
    ...
    if (index < rindex)
    {
        ...
        double tRi = devRi[rindex * numHiddenUnits + index];
        atomicAdd(&iPres, tmpHpr * tRi);
        atomicAdd(&iNext, tmphval * tRi);
     
        double tRf = devRf[rindex * numHiddenUnits + index];
        atomicAdd(&fPres, tmpHpr * tRf);
        atomicAdd(&fNext, tmphval * tRf);
     
        double tRg = devRg[rindex * numHiddenUnits + index];
        atomicAdd(&gPres, tmpHpr * tRg);
        atomicAdd(&gNext, tmphval * tRg);
     
        double tRo = devRo[rindex * numHiddenUnits + index];
    ...
    if (index == rindex)
    {
        ...
        iPres = sigmoid_elementwise(iPres + xi[rindex] + devBi[rindex]);
        fPres = sigmoid_elementwise(fPres + xf[rindex] + devBf[rindex]);
        gPres = tanh(gPres + xg[rindex] + devBg[rindex]);
        oPres = sigmoid_elementwise(oPres + xo[rindex] + devBo[rindex]);
     
        cval[rindex] = fPres * devCpr[rindex] + iPres * gPres;
        hval[rindex] = oPres * tanh(cval[rindex]);
     
        double tmphval = hval[rindex];
...
int main()
{
   printf(" Variant1 of Method 2 :-  GPU elementwise implementation  method :- \n");
   ...
   double *Y;
    ...
    printf("total time taken by gpu elementwise program (variant1) is %lf milliseconds for input size of %d , numHiddenUnits of %d and timeSteps of %d \n", total_time, inputSize, numHiddenUnits, timeSteps );
