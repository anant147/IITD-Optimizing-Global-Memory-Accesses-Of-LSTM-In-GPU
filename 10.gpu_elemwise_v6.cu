                                           ...
                                           int numHiddenUnits)
{
   ...
   __shared__ double tmpHpr;
   __shared__ double tmphval;
   ...
   for (int i=0;i<numHiddenUnits;i++)
   {
       if (index == i)
       {
           ...
           double tRi = devRi[i * numHiddenUnits + i];
           iPres[i] += tRi * tmpHpr;
        
           double tRf = devRf[i * numHiddenUnits + i];
           fPres[i] += tRf * tmpHpr;

           double tRg = devRg[i * numHiddenUnits + i];
           gPres[i] += tRg * tmpHpr;

           double tRo = devRo[i * numHiddenUnits + i];
           oPres[i] += tRo * tmpHpr;

           iPres[i] = sigmoid_elementwise(iPres[i] + xi[i] + devBi[i]);
           fPres[i] = sigmoid_elementwise(fPres[i] + xf[i] + devBf[i]);
           gPres[i] = tanh(gPres[i] + xg[i] + devBg[i]);
           oPres[i] = sigmoid_elementwise(oPres[i] + xo[i] + devBo[i]);

           cval[i] = fPres[i] * devCpr[i] + iPres[i] * gPres[i];
           hval[i] = oPres[i] * tanh(cval[i]);
       ...
       if (index > i)
       {
           ...
           double tRi = devRi[index * numHiddenUnits + i];
           iPres[index] += tRi * tmpHpr;
           iNext[index] += tRi * tmphval;

           double tRf = devRf[index * numHiddenUnits + i];
           fPres[index] += tRf * tmpHpr;
           fNext[index] += tRf * tmphval;

           double tRg = devRg[index * numHiddenUnits + i];
           gPres[index] += tRg * tmpHpr;
           gNext[index] += tRg * tmphval;

           double tRo = devRo[index * numHiddenUnits + i];
           oPres[index] += tRo * tmpHpr;
           oNext[index] += tRo * tmphval;
      }
                                           ...
                                           int numHiddenUnits)
{
    ...
    if (index == 0)
    {
        ...
        tmpHpr = devHpr[numHiddenUnits-1];
        tmphval = hval[numHiddenUnits-1];
    }

    __syncthreads();
 
    for (int i=numHiddenUnits-1;i>=1;i--)
    {
        if (index < i)
        {
            // double tmpHpr = devHpr[i];
        ...
        if (index == i-1)
        {
           iPres[index] = sigmoid_elementwise(iPres[index] + xi[index] + devBi[index]);
           fPres[index] = sigmoid_elementwise(fPres[index] + xf[index] + devBf[index]);
           gPres[index] = tanh(gPres[index] + xg[index] + devBg[index]);
           oPres[index] = sigmoid_elementwise(oPres[index] + xo[index] + devBo[index]);

           cval[index] = fPres[index] * devCpr[index] + iPres[index] * gPres[index];
           hval[index] = oPres[index] * tanh(cval[index]); 

           ival[index] = iNext[index];
           fval[index] = fNext[index];
           gval[index] = gNext[index];
           oval[index] = oNext[index];
...
int main()
{
   printf(" Variant6 (doing computation left to right (cond0) and right to left (cond1) and better use of shared memory) of Method 2 :-  GPU elementwise implementation  method :- \n");
   ...
   double *Y;
    ...
    printf("total time taken by gpu elementwise program (variant6 - doing computation left to right (cond0) and right to left (cond1) and better use of shared memory ) is %lf milliseconds for input size of %d , numHiddenUnits of %d and timeSteps of %d \n", total_time, inputSize, numHiddenUnits, timeSteps );
