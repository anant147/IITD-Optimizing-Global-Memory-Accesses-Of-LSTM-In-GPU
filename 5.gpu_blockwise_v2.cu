                                           ...
                                           int rCeil, int numHiddenUnits)
{
    ...
    if (index == 0)
    {
        for (int j=0;j<TILE_WIDTH;j++)
        {
            iPres[j] = 0.0; fPres[j] = 0.0; gPres[j] = 0.0; oPres[j] = 0.0;
            iNext[j] = 0.0; fNext[j] = 0.0; gNext[j] = 0.0; oNext[j] = 0.0;
        }
    }
   ...
   for (int i=0;i<rCeil; i++)
   {
        ...
        if (index == i)
        {
            ...
            addGateValue(iPres, ival, i, rCeil, numHiddenUnits);
            addGateValue(fPres, fval, i, rCeil, numHiddenUnits);
            addGateValue(gPres, gval, i, rCeil, numHiddenUnits);
            addGateValue(oPres, oval, i, rCeil, numHiddenUnits);


            sigmoid_blockwise(iPres, xi, devBi, i, rCeil, numHiddenUnits);
            sigmoid_blockwise(fPres, xf, devBf, i, rCeil, numHiddenUnits);
            tanh_blockwise(gPres, xg, devBg, i, rCeil, numHiddenUnits);
            sigmoid_blockwise(oPres, xo, devBo, i, rCeil, numHiddenUnits);
          
            cellAndHiddenVal_blockwise(hval, cval, devCpr, iPres, fPres, gPres, oPres, i, rCeil, numHiddenUnits);
            ...
            assignNextGateValue(ival, iNext, i, rCeil, numHiddenUnits);
            assignNextGateValue(fval, fNext, i, rCeil, numHiddenUnits);
            assignNextGateValue(gval, gNext, i, rCeil, numHiddenUnits);
            assignNextGateValue(oval, oNext, i, rCeil, numHiddenUnits);
...
int main()
{
   printf(" Variant2 (with for loop and two syncthreads) of Method 3 :-  GPU blockwise implementation  method :- \n");
   ...
   double *Y;
    ...
    printf("total time taken by gpu blockwise program (variant2 with for loop and 2 barriers in kernel function) is %lf milliseconds for input size of %d , numHiddenUnits of %d , timeSteps of %d and tileSize of %d \n", total_time, inputSize, numHiddenUnits, timeSteps, tileSize );
