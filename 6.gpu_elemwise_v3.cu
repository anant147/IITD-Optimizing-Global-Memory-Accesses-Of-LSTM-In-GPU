...
{
     double __shared__ Iweight[BLOCK_DIM], Fweight[BLOCK_DIM], Gweight[BLOCK_DIM], Oweight[BLOCK_DIM];

     int index = blockDim.x * blockIdx.x + threadIdx.x;
 
     if (index <= rindex)
     {
        Iweight[index] = devRi[rindex * numHiddenUnits + index];
        Fweight[index] = devRf[rindex * numHiddenUnits + index];
        Gweight[index] = devRg[rindex * numHiddenUnits + index];
        Oweight[index] = devRo[rindex * numHiddenUnits + index];
     }
     ...
     if (index == 0)
     {
          double  ipresent = ival[rindex];
          double  fpresent = fval[rindex];
          double  gpresent = gval[rindex];
          double  opresent = oval[rindex];
          double  inext = 0.0;
          double  fnext = 0.0;
          double  gnext = 0.0;
          double  onext = 0.0;
          ...
          for (int i=0;i<=rindex;i++)
          {
              ...
              ipresent += Iweight[i] * tmphpr;
              fpresent += Fweight[i] * tmphpr;
              gpresent += Gweight[i] * tmphpr;
              opresent += Oweight[i] * tmphpr;

              if (i == rindex)
              {
                ipresent = sigmoid_elementwise(ipresent + xi[i] + devBi[i]);
                fpresent = sigmoid_elementwise(fpresent + xf[i] + devBf[i]);
                gpresent = tanh(gpresent + xg[i] + devBg[i]);
                opresent = sigmoid_elementwise(opresent + xo[i] + devBo[i]);

                cval[rindex] = fpresent * devCpr[rindex] + ipresent * gpresent;
                hval[rindex] = opresent * tanh(kar har maidan fatehcval[rindex]);
              }

              double tmphval = hval[i];

              inext += Iweight[i] * tmphval;
              fnext += Fweight[i] * tmphval;
              gnext += Gweight[i] * tmphval;
              onext += Oweight[i] * tmphval;
          }
                                           ...
                                           int rindex, int numHiddenUnits)
{
   ...
   if (index == numHiddenUnits-1)
   {
        ...
        for (int i=numHiddenUnits-1;i>rindex;i--)
        {
            double tmphpr = devHpr[i];
            ipresent += Iweight[i] * tmphpr;
            fpresent += Fweight[i] * tmphpr;
        ...
        ipresent = sigmoid_elementwise(ipresent + xi[rindex] + devBi[rindex]);
        fpresent = sigmoid_elementwise(fpresent + xf[rindex] + devBf[rindex]);
        gpresent = tanh(gpresent + xg[rindex] + devBg[rindex]);
        opresent = sigmoid_elementwise(opresent + xo[rindex] + devBo[rindex]);

        cval[rindex] = fpresent * devCpr[rindex] + ipresent * gpresent;
        hval[rindex] = opresent * tanh(cval[rindex]);
   ...
   if (condition == 0)
   {
        ...
        for (int r=0;r<numHiddenUnits;r++)
        {
            ...
            gateValueComputation_Cond0<<< linBlockPerGrid , threadInBlock >>>(devHpr, devCpr, devRi, devRf, devRg, devRo, devBi, devBf, devBg, devBo,
                                                                              xi, xf, xg, xo, ival, fval, gval, oval, cval, hval, r, numHiddenUnits);
        }
   }
   else if (condition == 1)
   {
       ...
       {
           threadInBlock = numHiddenUnits-1-r;
           ...
           {
               outputElementComputation_Cond1<<< 1, 1 >>>( devCpr, xi, xf, xg, xo, devBi, devBf, devBg, devBo, ival, fval, gval, oval, cval, hval, r, numHiddenUnits);
           }
       }
   }
   else
...
int main()
{
   printf(" Variant3 (use of shared memory for storing weights) of Method 2 :-  GPU elementwise implementation  method :- \n");
   ...
   double *Y;
    ...
    printf("total time taken by gpu elementwise program (variant3) is %lf milliseconds for input size of %d , numHiddenUnits of %d and timeSteps of %d \n", total_time, inputSize, numHiddenUnits, timeSteps );
