                                           ...
                                           int numHiddenUnits)
{
    ...
    for (int i=0;i<numHiddenUnits;i++)
    {
        if (index <= i)
        {
           Iweight[index] = devRi[i * numHiddenUnits + index];
           Fweight[index] = devRf[i * numHiddenUnits + index];
           Gweight[index] = devRg[i * numHiddenUnits + index];
           Oweight[index] = devRo[i * numHiddenUnits + index];
        }
        ...
        if ( index == 0)
        {
            double  ipresent = ival[i];
            double  fpresent = fval[i];
            double  gpresent = gval[i];
            double  opresent = oval[i];
            double  inext = 0.0;
            double  fnext = 0.0;
            double  gnext = 0.0;
            double  onext = 0.0;
            ...
            for (int j=0;j<=i;j++)
            {
                double tmphpr = devHpr[j];
                ipresent += Iweight[j] * tmphpr;
                fpresent += Fweight[j] * tmphpr;
                gpresent += Gweight[j] * tmphpr;
                opresent += Oweight[j] * tmphpr;
                ...
                if (j==i)
                {
                  ...
                  cval[i] = fpresent * devCpr[i] + ipresent * gpresent;
                  hval[i] = opresent * tanh(cval[i]);
                }

                double tmphval = hval[j];
                inext += Iweight[j] * tmphval;
                fnext += Fweight[j] * tmphval;
                gnext += Gweight[j] * tmphval;
                onext += Oweight[j] * tmphval; 
            }
                                           ...
                                           int numHiddenUnits)
{
  ...
  for (int i = numHiddenUnits-2;i>=0;i--)
  {
       ...
       if (index == numHiddenUnits-1)
       {
            ...
            for (int j=numHiddenUnits-1;j>i;j--)
            {
                double tmphpr = devHpr[j];
                ipresent += Iweight[j] * tmphpr;
                fpresent += Fweight[j] * tmphpr;
...
int main()
{
   printf(" Variant4 (using for loop and shared memory in kernel function) of Method 2 :-  GPU elementwise implementation  method :- \n");
   ...
   double *Y;
    ...
    printf("total time taken by gpu elementwise program (variant4 - for loop and shared memory kernel func) is %lf milliseconds for input size of %d , numHiddenUnits of %d and timeSteps of %d \n", total_time, inputSize, numHiddenUnits, timeSteps );
