...
__global__ void gateValueComputation_Cond0(double *devHpr, double *devCpr, double *devRi, double *devRf, double *devRg, double *devRo,
                                           ...
                                           int rindex, int rCeil, int numHiddenUnits)
{
    ...
    __shared__ double iPres[TILE_WIDTH], fPres[TILE_WIDTH], gPres[TILE_WIDTH], oPres[TILE_WIDTH];  
    __shared__ double iNext[TILE_WIDTH] ,fNext[TILE_WIDTH] ,gNext[TILE_WIDTH] , oNext[TILE_WIDTH];
    ...
    if (index == 0)
    {
        for (int i=0;i<TILE_WIDTH;i++)
        {
            iPres[i] = 0.0; fPres[i] = 0.0; gPres[i] = 0.0; oPres[i] = 0.0;
            iNext[i] = 0.0; fNext[i] = 0.0; gNext[i] = 0.0; oNext[i] = 0.0;
        }
    }
   ...
   if (index < rindex)
   {
       ...
       // for (int i=0;i<TILE_WIDTH;i++)
       //{
         //  tmpHpr[i] = devHpr[index * TILE_WIDTH + i];
       //}
    
       getChunkOfArray(tmphval, hval, index, rCeil, numHiddenUnits);
       //for (int i=0;i<TILE_WIDTH;i++)
       //{
         //  tmphval[i] = hval[index * TILE_WIDTH + i];
       //}

       double tRi[TILE_WIDTH * TILE_WIDTH];
       getBlockOfArray2D( tRi, devRi, index, rindex, rCeil, numHiddenUnits);
       //for (int i=0;i<TILE_WIDTH;i++)
       //{
         //  for (int j=0;j<TILE_WIDTH;j++)
           //{
             //  tRi[i*TILE_WIDTH+j] = devRi[ ((rindex * TILE_WIDTH)+i) * numHiddenUnits + (index * TILE_WIDTH) + j];
           //}
       //}

       getBlockChunkMultiply(iPres, tRi, tmpHpr, index, rindex, rCeil, numHiddenUnits);
      ...
      //for (int i=0;i<TILE_WIDTH;i++)
       //{
         //  double sum = 0.0;
           //for (int j=0;j<TILE_WIDTH;j++)
           //{
             //  sum += tRi[i*TILE_WIDTH +j] * tmpHpr[j];
           //}
           //atomicAdd(&iPres[i], sum);
       // }
     
       getBlockChunkMultiply(iNext, tRi, tmphval, index, rindex, rCeil, numHiddenUnits);
    
       double tRf[TILE_WIDTH * TILE_WIDTH];
       getBlockOfArray2D( tRf, devRf, index, rindex, rCeil, numHiddenUnits);
       getBlockChunkMultiply(fPres, tRf, tmpHpr, index, rindex, rCeil, numHiddenUnits);
       getBlockChunkMultiply(fNext, tRf, tmphval, index, rindex, rCeil, numHiddenUnits);
    
       double tRg[TILE_WIDTH * TILE_WIDTH];
       getBlockOfArray2D( tRg, devRg, index, rindex, rCeil, numHiddenUnits);
       getBlockChunkMultiply(gPres, tRg, tmpHpr, index, rindex, rCeil, numHiddenUnits);
       getBlockChunkMultiply(gNext, tRg, tmphval, index, rindex, rCeil, numHiddenUnits);
    
       double tRo[TILE_WIDTH * TILE_WIDTH];
       getBlockOfArray2D( tRo, devRo, index, rindex, rCeil, numHiddenUnits);
       getBlockChunkMultiply(oPres, tRo, tmpHpr, index, rindex, rCeil, numHiddenUnits);
       getBlockChunkMultiply(oNext, tRo, tmphval, index, rindex, rCeil, numHiddenUnits);
   ...
   if (index == rindex)
   {
       ...
       //for (int i=0;i<TILE_WIDTH;i++)
       //{
         //  iPres[i] += ival[rindex * TILE_WIDTH + i];
       //}
       
       addGateValue(iPres, ival, rindex, rCeil, numHiddenUnits);
       addGateValue(fPres, fval, rindex, rCeil, numHiddenUnits);
       addGateValue(gPres, gval, rindex, rCeil, numHiddenUnits);
       addGateValue(oPres, oval, rindex, rCeil, numHiddenUnits);
    
       //int rval = rindex * TILE_WIDTH;
       //for (int i=0;i<TILE_WIDTH;i++)
       //{
         //  iPres[i] = sigmoid_elementwise(iPres[i] + xi[rval + i] + devBi[rval + i]);
       //}

       sigmoid_blockwise(iPres, xi, devBi, rindex, rCeil, numHiddenUnits);
       sigmoid_blockwise(fPres, xf, devBf, rindex, rCeil, numHiddenUnits);
       tanh_blockwise(gPres, xg, devBg, rindex, rCeil, numHiddenUnits);
       sigmoid_blockwise(oPres, xo, devBo, rindex, rCeil, numHiddenUnits);
    
       cellAndHiddenVal_blockwise(hval, cval, devCpr, iPres, fPres, gPres, oPres, rindex, rCeil, numHiddenUnits);
       ...
       assignNextGateValue(ival, iNext, rindex, rCeil, numHiddenUnits);
       assignNextGateValue(fval, fNext, rindex, rCeil, numHiddenUnits);
       assignNextGateValue(gval, gNext, rindex, rCeil, numHiddenUnits);
       assignNextGateValue(oval, oNext, rindex, rCeil, numHiddenUnits);
   }
}  
...
void LSTMForwardStepBlockReuse(double *x, GpuLstmLayer *gpulstmlayer)
{
   ...
   if (numHiddenUnits%TILE_WIDTH != 0 || TILE_WIDTH > numHiddenUnits)
   {
   ...
   else if (condition == 1)
   {
       ...
       for (int r=rCeil-1;r>=0;r--)
       {
           threadInBlock = rCeil-1-r;
           ...
           {
               outputElementComputation_Cond1<<< 1, 1 >>>( devCpr, xi, xf, xg, xo, devBi, devBf, devBg, devBo, ival, fval, gval, oval, cval, hval, r, rCeil, numHiddenUnits);
           }
       }
   }
   else
...
int main()
{
   printf(" Variant1 of Method 3 :-  GPU blockwise implementation  method :- \n");
   ...
   double *Y;
    ...
    printf("total time taken by gpu blockwise program (variant1) is %lf milliseconds for input size of %d , numHiddenUnits of %d , timeSteps of %d and tileSize of %d \n", total_time, inputSize, numHiddenUnits, timeSteps, tileSize );
