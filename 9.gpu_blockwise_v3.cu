...
int main()
{
   ...
   double *Y;
    ...
    printf("total time taken by gpu blockwise program (variant3 - doing computation left to right (cond0) and right to left (cond1)) is %lf milliseconds for input size of %d , numHiddenUnits of %d , timeSteps of %d and tileSize of %d \n", total_time, inputSize, numHiddenUnits, timeSteps, tileSize );
